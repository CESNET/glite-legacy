...
public:
	 ...
	 * \throw Exception Invalid value type for given attribute.
	 */
	QueryRecord(const Attr name, const Op op, const glite::lb_utils::jobid::JobId &value);
...
private:
	...
	glite::lb_utils::jobid::JobId jobid_value;
        int     int_value;
...
public:
	...
	void queryJobs(const std::vector<QueryRecord>& query,
		       std::vector<glite::lb_utils::jobid::JobId>& jobList) const;
	 ...
	 * \throws LoggingException Query failed.
	 */
	void userJobs(std::vector<glite::lb_utils::jobid::JobId> &jobs) const;

	/** Convenience form of userJobs.
	 *
	 * Same as above, but results are passed as a return value.
	 */
	const std::vector<glite::lb_utils::jobid::JobId> userJobs() const;
